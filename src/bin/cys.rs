...
// ★doctor --fix 는 이 항목을 절대 자동 수정하지 않는다 — 원래 봉인에 있던 파일을 건드리는
//   "부분 수리"는 ⓐ App Management(TCC) 보호에 막히고 ⓑ 지워도 added 가 missing 으로 바뀔 뿐
//   봉인은 여전히 깨진 채다. 복구는 두 갈래다(문안 정본 = APP_SEAL_RECOVERY, W3c):
//   어긋남이 전부 원래 설치에 없던 '추가(file added:)'뿐이면 **그 추가 파일 삭제만으로 봉인이
//   복구**되어 재설치가 필요 없고, 수정(modified)·누락(missing) 이 하나라도 있으면 번들 통째 교체뿐이다.
