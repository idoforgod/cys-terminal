...
왼쪽 그룹은 **아이콘 버튼**입니다(마우스를 올리면 툴팁에 이름·단축키가 뜹니다):
＋ 아이콘(새 창·⌘T) · 사각형+세로 분할선 아이콘(오른쪽 분할·⌘D) · 사각형+가로 분할선 아이콘(아래 분할·⌘⇧D) ·
`정렬`(역할 표준 배치) · × 아이콘(포커스 pane 즉시 닫기·확인 없음·⌘W) · 폴더 아이콘(파일 트리) ·
`Control Center`(승인 대기 배지) · `Update`(업데이트 배지) · `테마`.
좌측에 데몬 연결 상태가 표시됩니다.
...
- **pane 헤더의 × 버튼**은 **2-클릭 확인**(첫 클릭 후 2.5초 내 재클릭)입니다. **워크스페이스 삭제**(탭의 ×)는
  무엇이 어떻게 삭제되는지 설명하는 **확인 모달**이며(부서 탭이면 삭제 의도가 기록되어 재시작해도 부활하지 않습니다),
  그룹 삭제는 우클릭 메뉴의 2-클릭 확인입니다. 상단바의 × 아이콘(⌘W)은 확인 없이 포커스 pane을 즉시 닫습니다 — 두 ×는
  동작이 다릅니다.

**부서를 삭제했는데 탭이 남아 있다면** 결함이 아니라 설계된 동작입니다. 부서 삭제는 ①삭제 의도 기록
②pane 종료 ③부서 데몬 종료 순으로 진행되는데, ③이 실패하면(대개 부서 목록 파일이 손상돼 종료 완료를
확인할 수 없을 때) 그 탭을 **일부러 남깁니다** — 남은 탭이 곧 재시도 손잡이입니다. 탭에는 `종료 실패 —
탭을 다시 닫아 재시도 / 지금 켜기를 누르면 삭제가 취소됩니다` 라고 표시되며, 탭을 **다시 닫으면** 같은
종료를 재시도하고, 대신 `지금 켜기`를 누르면 **삭제가 취소**되어 부서가 되살아납니다(삭제 의도 기록이
해소됩니다). 앱을 다시 켜도 삭제 의도는 남아 있어 부서가 저절로 부활하지는 않습니다. 그래도 종료되지
않은 부서 데몬이 남아 있으면, 부서 목록을 복구한 뒤 터미널에서 `cys-dept down <부서명>`으로 정리하세요.
...
#### 껐다 켜면 어떻게 돌아오나 (v0.14.37)

앱은 켜질 때마다 **데몬에 실제로 있는 것**을 기준으로 화면을 맞춥니다. 저장된 배치(pane 분할·
탭 순서·이름)는 그 위에 얹히는 **기억**일 뿐, "무엇이 존재하는가"의 기준이 아닙니다.

- **본부(기본) 워크스페이스 탭은 켤 때마다 반드시 있습니다.** 탭을 닫으셨더라도 다음 기동에
  다시 생깁니다 — 기본 데몬은 삭제 개념이 없고 CEO·마스터가 그 안에 살기 때문입니다.
- **레지스트리에 등재된 부서는 저장된 배치에 없어도 탭이 생깁니다.** CLI·에이전트가 GUI 밖에서
  만든 부서도 다음 기동에 보입니다. **삭제하신 부서(묘비)는 되살아나지 않습니다.** 삭제 기록을
  읽지 못하면(데몬 무응답 등) 그 회차에는 **부서 탭을 아예 만들지 않습니다** — 지운 부서가
  되살아나는 쪽보다 한 박자 늦게 뜨는 쪽이 안전하기 때문입니다.
- **한 번에 너무 많은 팀을 띄우지는 않습니다.** 탭은 전부 생기지만, 데몬과 5명 팀을 실제로
  켜는 것은 **쓰시던 탭 먼저**이고 그 밖은 회차당 몇 개까지입니다. 미룬 부서는 탭에 안내와
  [지금 켜기] 버튼이 뜨고, 앱을 다시 켜면 이어서 준비됩니다. (이 **회차 상한**은 앱 기동 시
  복원에만 적용됩니다 — 업데이트 직후 한 번 도는 조직 복원에는 걸리지 않습니다. 다만 위의
  삭제 기록 규칙은 그 경로에도 같이 적용됩니다.)
- 부서가 많아 복원이 길어지면 **탭 목록을 먼저 그리고** 내용은 이어서 채웁니다. 정해진 시간
  예산을 넘기면 그 시점 상태로 화면을 세우고 나머지는 3초 자동 복구가 맡습니다.
- 재부팅하면 데몬이 팀을 **새 pane 번호로** 되살립니다. 앱은 그 번호를 찾아 자동으로 붙입니다
  (기동 직후 몇 초 안에, 늦게 깨어나는 노드는 3초 주기로 계속 붙습니다).
- 데몬이 대답하지 않는 워크스페이스의 탭은 **지우지 않고 보존**합니다. 일시적으로 멈춘 것을
  삭제로 오해하지 않기 위해서입니다. 데몬 하나가 먹통이어도 나머지 탭은 정상 복원됩니다.
- 앱이 켜져 있는 동안 종료된 pane 은 `[exited]` 로 남겨 마지막 출력을 읽으실 수 있습니다. 다만
  **앱을 껐다 켜면 그 pane 은 사라집니다** — 출력을 보여 줄 터미널이 새로 뜨기 때문입니다(기록은
  트랜스크립트에 남습니다). 데몬이 기록조차 모르는 pane(재기동으로 소멸)도 정리됩니다.
- 창이 하나도 열리지 못한 탭은 **백지가 아니라 안내와 손잡이**를 보여 줍니다 — 부서는
  [지금 켜기], 본부는 [새 셸 열기] 입니다.
...
폴더 아이콘(파일 트리) 버튼 — 포커스 pane의 현재 디렉터리를 루트로 트리를 보여주고(cd 추적), 헤더
