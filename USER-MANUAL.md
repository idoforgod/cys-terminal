...
- macOS(권장): Control Center 헤더 → **"셸에 cys 설치"** 1클릭(관리자 승인 1회) — `/usr/local/bin/cys`·`cysd` 심볼릭 링크가 생기고, 앱 업데이트에도 자동 추종합니다. 이 버튼은 macOS에서만 나타납니다.
  ...
  - **해제 확인 창은 설치 때 백업해 둔 원본이 있으면 그 경로와 되돌리는 방법을 함께 보여 줍니다.** 그리고 **해제는 그 원본을 제자리에 되돌립니다**(우리 링크를 지운 자리에만 · 남의 파일 위에는 덮지 않습니다). 되돌린 경로는 결과 알림에 나오고, 되돌리지 못하고 남은 백업본은 계속 고지됩니다. **그 고지에 붙는 되돌리기 명령은 `sudo mv -n …`(덮어쓰기 금지) 이고, 원래 자리가 비어 있어야 옮겨집니다 — 비어 있지 않으면 아무 일도 일어나지 않습니다** (예: `sudo mv -n /usr/local/bin/cys.cys-backup-1756089600 /usr/local/bin/cys`). 그리고 그 자리를 이 앱의 링크가 차지하고 있다고 앱이 아는 경우에는 옮기는 명령을 **아예 내지 않고** "해제를 누르면 앱이 되돌립니다"라고만 말합니다. 자리 상태를 확정하지 못하면 `ls -l <원래 경로>` 로 먼저 확인하라고 안내합니다. 백업이 없으면 그 문장은 아예 나오지 않습니다.
