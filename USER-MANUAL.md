...
macOS 는 데스크탑·문서·다운로드 같은 폴더를 앱마다 따로 허락합니다(시스템 설정의 「파일 및 폴더」). 이 허락이 꺼져 있으면 팀 자리(좌석)의 AI 가 그 폴더의 파일을 읽지도 쓰지도 못합니다. 이때 cys 는 조용히 넘어가지 않고 다음처럼 알립니다.

- **알림 두 가지**
  - 앱을 열 때 앱이 데스크탑·문서 폴더를 직접 읽어 봅니다. 막혀 있으면 "⚠ macOS 데스크탑 폴더 접근이 꺼져 있습니다" 알림이 뜹니다.
  - 팀 자리의 **작업 폴더**가 막혀 있으면 "⚠ 팀 자리가 데스크탑 폴더를 읽지 못합니다" 알림이 그 자리의 역할 이름(예: worker-2)과 함께 뜹니다. 같은 폴더의 자리는 한 장으로 묶입니다.
  - **알림을 누르면 시스템 설정의 「파일 및 폴더」 화면이 열립니다.** 알림은 10분 뒤 자동으로 닫히고, Control Center 의 **알람 탭**에서 다시 볼 수 있습니다.
  - 파일 트리에서 막힌 폴더를 펼치면 빈 폴더처럼 보이지 않고 "⚠ macOS 폴더 접근이 막혀 읽을 수 없습니다" 한 줄이 보입니다(눌러도 같은 화면이 열립니다).
- **목록에서 찾을 이름은 「cys」(앱 아이콘) 하나입니다.** 팀 자리의 claude·python·데몬(cysd)이 폴더를 읽어도 macOS 는 전부 앱 「cys」의 허락으로 봅니다(macOS 27 에서 확인). 옛 macOS 에서 「cysd」가 따로 보이면 그것도 함께 켜세요(macOS 13–15 에서는 확인하지 못했습니다).
- **켜는 곳**: 시스템 설정 → 개인정보 보호 및 보안 → **파일 및 폴더** → 「cys」 → 데스크탑 폴더(또는 문서 폴더·다운로드 폴더) 스위치. macOS 12 이하는 "시스템 환경설정 → 보안 및 개인 정보 보호 → 개인 정보 보호 → 파일 및 폴더" 입니다. 화면 배치는 macOS 버전마다 조금 다릅니다 — 알림을 누르면 그 화면이 바로 열립니다.
- **켠 뒤**: 팀 자리의 "막힘" 표시는 **자리가 만들어질 때 한 번 재는 값**입니다. 허락을 켠 뒤에는 막혔던 자리를 다시 띄우거나 컴퓨터를 다시 시작하면 표시가 사라집니다.
- **팀 자리는 그대로 뜹니다.** 폴더가 막혀도 자리는 그 폴더에서 뜹니다(홈으로 옮기지 않습니다). 대신 자리에 "이 좌석이 시작될 때 작업 폴더를 읽을 수 없었다"는 고지가 한 줄 들어가고, 그 폴더가 필요한 지시를 받으면 **그 지시의 회신에만** 그 사실을 적습니다. 다른 폴더에 대신 저장하거나 스스로 반복 보고하지 않습니다.
- **터미널에서 cys 데몬을 직접 띄웠다면**: 그 데몬이 띄운 자리는 「cys」가 아니라 그 **터미널 앱**(예: 터미널·iTerm)의 허락을 씁니다. 그때는 목록에서 그 터미널 앱을 켜야 합니다.
- **같은 막힌 폴더에 여러 자리를 동시에 만들면 일부 자리만 표시될 수 있습니다.** 폭주 방지를 위해 같은 폴더 점검이 겹치면 뒤 자리는 판정을 건너뜁니다(동작은 바뀌지 않습니다 — 그 자리도 그대로 뜹니다). 표시가 빠진 자리가 있으면 그 자리만 다시 만들어 보세요.
- **전체 디스크 접근 권한은 최후 수단입니다.** 「파일 및 폴더」 목록에 「cys」가 아예 보이지 않을 때만 고려하세요. 전체 디스크 접근은 메일·메시지·Safari·Time Machine 백업까지 여는 넓은 권한이고, 이 앱은 AI 가 확인 없이 명령을 실행하는 자리를 띄웁니다. 그래도 켜야 한다면: 시스템 설정 → 개인정보 보호 및 보안 → 전체 디스크 접근 권한 → ＋(추가) → 응용 프로그램 → cys.
- **데몬이 안 뜰 때(로그인 항목)**: 시스템 설정 → 일반 → 로그인 항목(macOS 15 이상은 "로그인 항목 및 확장 프로그램") → **「백그라운드에서 허용」**에서 「cys」와 **개발자 이름 줄(「yoonsik choi」 — 앱 서명자 이름)**을 모두 켜세요. 데몬(cysd) 스위치는 앱 줄이 아니라 개발자 이름 줄에 붙어 보일 수 있습니다. "데몬 시작 대기 중" 알림을 누르면 이 화면이 열립니다.
- 끄기(진단용): 데몬 환경변수 `CYS_CWD_PROBE=0` 이면 팀 자리 작업 폴더 관측을 하지 않습니다(§16).
...
- `정렬` 버튼: 역할 기반 표준 배치(좌측 master/CSO · 가운데 worker · 우측 리뷰어). **대표(master) 칸은 화면(pane
  영역)의 1/3** 이고 나머지 좌석이 2/3 를 같은 폭으로 나눕니다. 정렬은 직접 끌어 둔 비율을 표준으로 되돌립니다.
- **새 좌석이 생겨도 대표 칸은 1/3 을 유지합니다**(0.14.41~). 종전에는 좌석이 하나 늘 때마다 대표 칸이 절반씩
  줄었습니다(1/2 → 1/4 → 1/8). 이제 새 좌석은 대표 칸 오른쪽의 2/3 안에서만 자리를 나눠 갖습니다. 대표가 없는
  탭(일반 셸 탭)은 종전처럼 오른쪽에 붙습니다.
- **분할선을 직접 끌었거나 pane 을 드래그해 옮긴 탭**은 사용자가 정한 배치로 기억해 자동으로 다시 짜지 않습니다
  (그래도 새 좌석 때문에 대표 칸이 줄지는 않습니다 — 단, **대표 칸을 화면 왼쪽 밖으로 직접 옮긴 탭**은 예외로,
  그 뒤 처음 생기는 좌석 하나에 한해 대표 칸도 다른 칸과 함께 균등한 몫으로 줄고, 그 다음부터는 다시 줄지
  않습니다). 표준으로 돌아가려면 `정렬`을 누르세요.
- **같은 부서 탭 사이의 pane 전출**(위 "pane 전출" 참고)은 이 1/3 유지 약속의 **예외**입니다 — 목적지 탭에서
  대표 칸이 절반씩 부착되는 종전 규칙으로 받아들여, 다음 자동 배치(3초 입양·재부팅 등)가 있을 때까지 대표
  칸이 일시적으로 좁아질 수 있습니다.
- **재부팅·업데이트 뒤에도 역할별 자리를 기억합니다**(0.14.41~). 각 칸이 어느 역할(master·cso·worker·리뷰어)의
  자리였는지 함께 저장해 두었다가, 데몬이 좌석을 새 번호로 되살리면 같은 역할의 창을 **원래 칸**에 다시 넣습니다.
  복원이 진행 중이면 아직 안 온 역할의 칸에 "대표(master) 자리 — 복원을 기다리는 중입니다"와 스피너가 최대 4분
  (Windows 8분) 보이고, 그 칸의 **[새 셸 열기]**(그 자리에서 셸 열기)·**[칸 비우기]**(자리 기억 지우기) 버튼으로 바로
  정리할 수 있습니다. 기한이 지나면 화면에서만 접히고 자리 기억은 남아, 늦게 돌아온 역할도 제자리로 갑니다.
  ⚠ 업그레이드 직후 첫 기동은 아직 역할 기억이 없어 표준 배치(대표 왼쪽 1/3)로 시작하고, 그 다음 재부팅부터
  원래 칸으로 돌아옵니다.
...
탭 앞의 점은 그 워크스페이스 좌석들의 상태입니다(v0.14.41부터 **모르는 것은 모른다고** 표시합니다).
점이 회색·빈 자리·빨강이거나 둘째 줄에 `미등록 N`이 보일 때 그 줄에 마우스를 올리면 이유가 나옵니다
(초록·노랑은 따로 설명이 붙지 않습니다).

| 점 | 뜻 |
|---|---|
| 초록 | 최근 30초 안에 받은 신호가 모두 "일하는 중"입니다. |
| 노랑 · `💤N` | 조용한(유휴) 좌석이 N개 있습니다. |
| 빨강 · `❌N` | 에이전트가 끝난 것이 확인된 좌석이 N개 있습니다. |
| 노란 테두리 원 · `○빈자리 N` | 역할 이름표는 있는데 에이전트가 떠 있지 않은 **빈 자리**가 N개 있습니다(재시작 뒤 에이전트가 뜨지 못한 경우 등). 좌석 화면을 열어 확인하세요. 표시만 하며 자동으로 닫거나 다시 띄우지 않습니다. |
| 회색 · `상태 수집 전` / `상태 미확인` / `?미확인 N` | 데몬에게서 상태를 아직 받지 못했거나 30초 넘게 새 신호가 없습니다(Windows는 60초). 초록으로 칠하지 않고 "확인 못 함"으로 둡니다. |
| 회색 · `좌석 없음` | 그 탭에 pane 이 하나도 없습니다(데몬 문제가 아닙니다). |
| (점 색 그대로) · `미등록 N` | 역할 이름표는 있는데 등록된 에이전트가 없고 셸 아래 프로세스도 보이지 않는 좌석이 N개 있습니다. 명령을 직접 띄운 감시 pane 이면 정상이고, 에이전트를 띄우기 전의 빈 셸일 수도 있습니다 — 앱이 둘을 가려낼 수 없어서 **빈 자리로 판정하지 않고** 사실만 적습니다. |

빈 자리 표시는 좌석이 생긴 지 60초가 지나고 20초 넘게 출력이 없을 때만 붙습니다(Windows는 두 값 모두 2배).
에이전트가 방금 상태를 보고했다면 빈 자리로 보지 않습니다. Control Center **Tasks** 탭도 같은 규칙입니다 —
빈 자리는 `빈 자리`, 10분 넘게 갱신되지 않은 자기보고는 출력 활동 기준 상태에 `(자기보고 낡음)`을 붙여 보여 줍니다.

Control Center 머리의 경보 배지도 같은 원칙입니다. 경보 조회가 3번 연속 실패하면 "경보 0"처럼
숨기지 않고 회색 **`⚠ 경보 조회 불가`** 로 보여 줍니다(Control Center 를 다시 열면 실패 횟수를 새로 셉니다).

#### 사이드바 바닥: 사용량 · 전문가용 (v0.14.41)

사이드바 맨 아래에 **사용량** 칸과 **전문가용** 칸이 고정됩니다(탭이 많아도 밀려나지 않고, 칸이 길어지면
그 안에서 스크롤됩니다).

- **사용량**: 계정별 **5시간·7일 한도 사용률**을 Control Center 를 열지 않고 봅니다. 맨 위는 **주 계정**
  (cys 창에서 지금 쓰이고 있는 계정 가운데 5시간 사용률이 가장 높은 계정) — 사용률·게이지·리셋 시각,
  "이 속도면 HH:MM 소진" 예측이 나옵니다. 그 아래에 다른 관측 계정이 한 줄씩, 한 번도 관측되지 않은
  계정은 개수만 나옵니다. 머리줄을 누르면 한 줄 요약(`5h 78% · 7d 31%`)으로 접힙니다.
  - 표시 규칙: 70% 이상 주황·90% 이상 빨강, 100%를 넘으면 `100%+`, 그 창의 기록이 없으면 `—`(0%가
    아닙니다), 리셋 시각이 지난 값은 숨기고 "리셋됨"으로, 30분 넘게 새 관측이 없거나 지난 기록이면 흐리게.
    데몬이 세 번 연속 응답하지 않으면 "데몬 응답 없음 — HH:MM 기준 값"으로 표시합니다.
  - 이름은 설정 폴더 이름(`claude-4`·`좌석`·`부서 dept-1`)으로만 보입니다. 이메일은 마우스를 올렸을 때의
    설명에만 나오며, Control Center 의 🔒 가림 토글을 켜면 그곳도 가려집니다.
    **★이 폴더 이름 자체는 🔒 가림 대상이 아닙니다** — 계정 폴더를 `.claude-<사람 이름>`처럼 개인
  ...
  - **집계 범위**: cys 창 안에서 **계정 전용 설정 폴더(CLAUDE_CONFIG_DIR)로 띄운** 세션만 계정에 모입니다.
    기본 `claude`(`~/.claude`)로 띄운 세션과 외부 터미널 세션은 집계되지 않습니다.
...
사이드바 바닥 **전문가용 ▸ 팀 직접 만들기**(또는 명령 팔레트 ⌘K "팀 직접 만들기")로 부서(팀)
워크스페이스를 만들면 **별도 cysd 데몬(별도 소켓)**이 뜹니다 — 프로젝트 간 장애·자원·통신이
격리됩니다. Control Center "작업" 탭과 `cys fleet`은 모든 부서를 집계해 보여줍니다.

- **★만들기 전에 확인 창이 한 번 뜹니다 (v0.14.41)**: 종전 `＋부서` 버튼은 워크스페이스 `＋` 바로 옆에
  있어 잘못 누르기 쉬웠고, 카탈로그가 없으면 한 번 누르는 즉시 부서를 만들었습니다. 이제 버튼은
  전문가용 칸 안에 있고, 만들기 전에 **무엇이 만들어지는지**(팀 이름·자리 최대 5개·작업 폴더·첫 로그인
  필요 가능성·첫 팀이면 본부 대표의 CEO 전환)를 보여 주는 확인 창이 한 번 뜹니다. 기본 선택은 **[취소]**
  이며, 취소하면 아무것도 만들지 않습니다. 이미 등록된 팀을 고르면 "팀 다시 열기" 창이 뜹니다(꺼져
  있으면 다시 켜고 자리를 다시 띄웁니다). 고른 팀의 카탈로그를 찾지 못하면 번호 팀으로 **자동으로**
  만들지 않고 한 번 더 묻습니다.
...
- **마스터 선언 → 부서 자동 생성(위계 폴백)**: 이미 살아있는 마스터(CEO)가 있는 조직에서
  **오너가 직접 타이핑한** "너는 마스터다" 선언(base 레인·unix)은 거부로 끝나지 않고,
  ...
  않습니다(폭주 봉인 — 거부 안내만).
- **★GUI 버튼(현 전문가용 ▸ 팀 직접 만들기)으로 만든 부서도 팀이 자동으로 뜹니다 (v0.14.23)**: 부서의 팀 기동에는
...
- **★말로 팀 만들기 (v0.14.41 · 1차)**: 본부 대표(마스터·CEO)에게 "○○하는 팀 하나 만들어줘"라고
  ...
  - **★팀이 아직 하나도 없는 기존 설치는 이 절차를 모를 수 있습니다**: 위 절차(`cys team-propose`)는
    대표 지침 파일(`MASTER_DIRECTIVE.md`)에 실려 있는데, 이 파일은 오너가 직접 고쳤을 수도 있는
    **사용자 소유 파일**이라 새 버전이 나와도 기존 설치에는 자동으로 덮이지 않고 `.new`(병합 대기)로만
    놓입니다. CEO(부서를 이미 하나 이상 만든 대표)는 승격 시 최신 지침을 새로 받아 이 절차를 알지만,
    **부서가 아직 0개인 기존 설치의 본부 대표는 구지침 그대로일 수 있습니다.** 대표에게 말해도
    반응이 없거나 절차를 모르는 것 같으면, `cys pack-merge`(병합 대기 목록 확인) →
    `cys pack-merge --file cysjavis-pack/directives/MASTER_DIRECTIVE.md --take-new`(또는 직접 수정이
    없으면 기본 3-way 자동 병합)로 새 대표 지침을 받아야 이 기능이 켜집니다.
...
### 4.11 피드백 보내기 (v0.14.41)

왼쪽 워크스페이스 목록 아래의 **피드백 보내기** 단추를 누르면 작성 창이 열립니다.

1. **무엇을 하다가 어떤 일이 있었는지** 5자 이상 적습니다(필수).
2. 사진·영상을 넣습니다(선택 · 최대 10개 · 파일 하나 200MB · 합계 300MB).
   - 창에 **끌어다 놓기**, **[파일 고르기]**, 캡처한 뒤 **⌘V(윈도우 Ctrl+V) 붙여넣기** 모두 됩니다.
   - 캡처 요령: 맥 ⌘⇧4(화면 일부 사진)·⌘⇧5(화면 녹화) / 윈도우 Win+Shift+S(사진)·캡처 도구 녹화(영상).
   - 받는 형식: png·jpg·jpeg·gif·webp·heic·mov·mp4·m4v·webm·txt·log·json.
3. **진단 정보 함께 넣기**(기본 켜짐): 앱·운영체제 버전, 데몬 응답 여부, 화면 수만 넣습니다. 개인 폴더
   경로는 `~`·`<user>` 로 가리고, **[넣을 진단 내용 보기]** 에 보이는 내용이 그대로 저장됩니다. 끄면 넣지 않습니다.
4. **[메일로 보내기]** 를 누르면
   - 이 컴퓨터의 `~/.cys/feedback/<번호>/` 에 묶음(설명 `description.txt`·첨부·`diag.json`·`manifest.json`)을 만들고,
   - 그 폴더를 열고, 기본 메일 앱을 **받는 주소·제목·내용이 채워진 채로** 엽니다.
   - 첨부 파일은 열린 폴더에서 메일 창으로 끌어다 넣은 뒤 메일에서 **[보내기]** 를 누르세요.
     맥에서 Mail 앱을 쓰시면 결과 화면의 **[Mail 앱에 첨부해 열기]** 로 파일이 붙은 새 메시지를 열 수도 있습니다
     (받는 사람·제목은 직접 적어야 합니다).
   - 메일 앱이 열리지 않으면 결과 화면의 받는 주소를 **[주소 복사]** 로 복사해 직접 보내 주세요.

알아 둘 점:
- **cys 는 자동으로 아무것도 보내지 않습니다.** 메일에서 직접 [보내기]를 누르실 때만 전달됩니다(서버 업로드 없음).
- 터미널 화면 내용·대화 기록·파일 목록은 모으지 않습니다. 첨부할 화면에 비밀번호·API 키·개인 정보가 보이지 않는지 확인해 주세요.
- 보내기 전에 창을 닫으면(버리기) 넣어 둔 첨부 사본은 지워집니다. 만든 묶음은 남으며, **완전 초기화** 때 다른 사용 흔적과 함께 격리됩니다.
- 피드백 내용은 에이전트(마스터·워커)에게 전달되지 않습니다.
...
- `schedule list` 각 줄 끝의 `result=` 칸은 **데몬 기동 이후 마지막 발화 결과**입니다(0.14.41):
  ...
  연속 실패(`error`) `FAILING×N` · 연속 시간초과(`timeout`) `TIMEOUT×N`(0.14.41 수정1 — error 와
  다른 딱지: 상한 만료는 응답이 늦었다는 뜻이지 명령이 실패했다는 확정이 아니라서 나눴습니다) ·
  마지막 `ok` 이후 연속으로 일을 안 한 횟수 `non_ok×N`. `last_fired` 는 **발화 시각**(발화 전에
...
`Update` 를 누르면 **늘 상태 창**이 열립니다 — 다시 확인한 뒤 본체·팩 각각의 현재→새 버전(또는
실패 사유), 마지막 확인 시각, 설치 버튼을 보여 줍니다. 배지는 개수가 아니라 **기호**입니다.

| 배지 | 뜻 | 동작 |
|---|---|---|
| `↻` | 팩 변경 (무중단) — 새 본체도 함께 있으면 팩이 먼저 | 창의 [팩 무중단 적용] → 서명 검증 → 원자 반영 → 라이브 노드 재주입. **재시작 0, 세션·데몬 생존** |
| `!`(빨강) | 앱(바이너리) 업데이트, 또는 새 팩이 더 새 본체를 요구 | 창의 [본체 패치 설치] → 확인 → 다운로드·설치 → 재시작 → 팩 반영+노드 자동 복귀 |
| `!`(호박색) | **pro 채널** 설치 — 앱은 pro 업데이트를 확인하지 않습니다. 공개(free) 팩이 이 설치의 base 보다 새것일 때만 뜨고, 조치는 필요 없습니다(공개 팩은 애초에 적용 대상이 아님) | 창에서 사유 확인 · 설치 버튼 없음. pro→free 전환은 `cys pack-downgrade-to-free` 전용 명령만 |
| `✓` | 최신 — 본체·팩 **두 확인이 모두 성공**했고 대기 0건 | 창에 두 버전과 확인 시각 |
| `?` | 최신 여부를 알 수 없음 — 확인 실패(네트워크)·팩 정보 해석 불가·설치된 팩 상태 불명 | 창에서 사유 확인 · [다시 확인]. 직전에 검증된 업데이트가 있으면 '직전 확인 기준'으로 설치 버튼을 유지 |
| `…` | 확인 중 | — |

이미 적용된 팩을 다시 적용하면 "✅ 완료"가 아니라 **"이미 적용돼 있음"**(설치된 팩 버전을 읽지 못했으면
"팩 상태 불명")으로 알립니다.
...
| `CYS_CWD_PROBE` | 켜짐 (`0`=끔 · `CYS_BOOT_GATES=0` 도 끔) | **macOS 전용** — 역할 좌석을 만들 때 작업 폴더 목록 읽기를 1회(최대 1.5초) 재서, macOS 폴더 접근 권한 거부(EPERM)면 `surface.list`/`org.status` 에 `cwd_blocked` 를, 좌석 env 에 `CYS_CWD_BLOCKED` 를 싣습니다(§2.1a). 스폰 동작은 바꾸지 않습니다 |
