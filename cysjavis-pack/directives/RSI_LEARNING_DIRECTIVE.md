...
추천은 **다이제스트 큐에 적재**한다(`<pack>/round/learn/digest_queue.jsonl`) — 라운드 종료·
ceiling 같은 **자동 트리거는 건별 feed 승인 요청을 발행하지 않는다**. 오너에게는 §7-4 접점
2등급대로 **주간 다이제스트 1건**으로 모아 간다. 사람이 실제로 착수를 원할 때만
`javis_learn propose` 로 항목을 만들고 그때 `feed.push --wait` 승인요청을 낸다.
**승인(0)에서만** ①~⑤ 착수(거부·타임아웃=무실행).
<!-- 개정 근거: 오너 승인(2026-09-04 전면 감사). 승인권은 오너뿐인데 자동 생성된 learn_proposal
     6건이 최고령 19h48m 적체했고, 그 소음이 **실제 승인 요청**(SSH 프로브 9.5h)을 덮었다.
     §7-4 의 "접점 신설 시 다이제스트가 기본값(승인 피로 봉쇄 불변식)"이 이미 정본이므로,
     이 개정은 새 규범을 만드는 것이 아니라 **코드가 그 정본을 따르게 하는 정합 수정**이다. -->
