...
- 판정의 단일 소유자는 `javis_mission.py` 다(exit 0=임무 있음 / 1=없음 / **2=판독 불가는 '없음'과
  ...
  ① 환경변수 `CYS_MISSION` 이 비어 있지 않다(기동 시점 명시 지정)
  ② 이 레인의 임무 대장에 오너 임무가 기록돼 있고 `surface` 가 이 pane 과 일치한다
- 대장은 **UserPromptSubmit 훅**이 오너 프롬프트에서 자동 기록한다(네가 아니라 하네스가 관측한다).
  마스터 선언 프롬프트는 대장을 **재개장**하므로, "너는 마스터다" 단독 부팅은 항상 **임무 없음**이다.
- 오너가 구두로 임무를 준 뒤 대장이 비어 있으면(짧은 승낙 등) 그때만
  `python3 "${CYS_PACK_DIR:-$HOME/.cys/pack}/bin/javis_mission.py" set "<임무>"` 로 기록한다 —
  **오너 발화 없이 이 명령을 부르는 것은 게이트 우회이며 금지**다.
