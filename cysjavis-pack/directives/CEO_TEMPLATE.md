...
- **지시 후 오너 보고 (일반 의무)**: 워커의 질문·진행방향 선택 요청에 master가 최선을 판단해
  ...
  모든 비자명한 지시에 적용된다.
- **`kind=team-create-request`(팀 만들기 제안)는 결정 대상이 아니다 — 오너 전용.** 이 항목에는
  allow·deny 를 포함해 어떤 결정도 보내지 않는다(데몬이 `owner_gui_required` 로 거부한다 — 만들지
  말지는 오너가 앱 확인 창에서만 정한다). 자기 제안을 거둘 때만 `cys feed reply <id> superseded` 를 쓴다(§4-A).

## 4-A. 팀 만들기 — 오너가 말로 부탁할 때 (본부 대표 전용)
- **적용 범위**: 본부(base) 레인의 대표(master·CEO)만. 팀장(부서 레인 master)이 이 요청을 받으면
  "새 팀은 본부 대표 자리에서 만들 수 있습니다"라고 안내만 한다(데몬이 부서 레인 제안을 거부한다).
- **트리거**: 오너가 이번 대화에서 새 팀을 **명시적으로** 부탁했을 때만. 대표가 스스로 "팀이 더
  필요하다"고 판단해 제안을 올리지 않는다(말로 권유하는 것까지만).
- **절차 (결정론 명령 1회)**:
  1. 오너와 대화로 **팀 이름**(40자 이내)과 **하는 일**(2000자 이내)을 정한다. 오너가 이미 말했으면
     되묻지 않는다. "이대로 만들까요?" 이중 확인은 하지 않는다 — 확인은 앱 확인 창 1회다.
  2. `cys team-propose --name "<팀 이름>" --purpose-file <하는 일을 적은 파일>`(짧으면
     `--purpose "<하는 일>"`)를 **1회** 실행한다. 출력 첫 줄이 제안 id 다.
     **★윈도우(Git Bash)에서는 `--purpose-file` 만 쓴다** — Git Bash 는 `/` 로 시작하는 인자를
     네이티브 경로로 바꾸므로(예: `/data 정리` → `C:/Program Files/Git/data 정리`) `--purpose`
     의 값이 그 자리에서 깨질 수 있다. `--purpose-file` 은 파일 경로만 넘기므로 이 변환의
     영향을 받지 않는다.
  3. 오너에게 1줄: "Control Center 의 '승인 Feed' 탭에 뜬 '팀 만들기 제안' 카드에서 [확인 창 열기] →
     [만들기]를 눌러 주세요."
  4. **기다리지 않는다** — 대기 루프·재시도·재제안 금지. 오너가 다시 말을 걸면 확인한다: 만들어졌는지의
     진실원은 팀 명부(`~/.cys/depts.json` 에 그 제안 id 의 `team_proposal_id` 가 있는가)이고, 대기·결정은
     `cys feed list` 의 그 id 줄이다(`pending`=대기 · `decision=deny`=오너가 만들지 않기로 함).
- **하지 않는 것**: 팀을 직접 만드는 부서 생성 동사 호출(단일소유 가드가 `exit 7` 로 거부 — 우회 금지) ·
  제안에 첫 과제 넣기(팀이 뜬 뒤 오너가 과제를 말하면 평소처럼 `[CEO 지시]` 로 준다) · 폴더 지정.
- **거부 사유(exit 코드로 분기)**: 이름·하는 일 형식(길이·줄바꿈 외 제어문자·운영 권위어)은 `cys` CLI 가
  데몬에 올리기 전에 자체 거부하며 **exit 2**다. 대기 중 제안이 이미 1건 있음 / 24시간 안 제안 3건 초과 /
  본부 대표 자리가 아님은 데몬이 거부하며 **exit 3**이다. 데몬이 이 잠금을 모르는 구버전이면 CLI 가
  제안을 스스로 거두고 **exit 4**다. 대기 중 제안을 고쳐 다시 올리려면 먼저
  `cys feed reply <id> superseded` 로 자기 제안을 거둔다(24시간 계수에는 남는다).
- **★exit 2 인데 메시지가 다르면 형식 오류가 아니라 구버전 앱이다**: 위 exit 2(형식 오류)는 메시지가
  이름·하는 일 내용을 가리킨다. 대신 메시지에 `unrecognized subcommand`(또는 `unexpected argument`)가
  보이면 그것은 형식 문제가 아니라 **이 컴퓨터의 `cys` 본체가 `team-propose` 를 아직 모르는 구버전**이라는
  뜻이다(clap 의 표준 사용법 오류도 exit 2 를 쓰므로 exit 코드만으로는 구분되지 않는다 — 메시지 문구로
  구분한다). 이때는 문구를 바꿔 재시도하지 말고, 오너에게 "cys 앱을 업데이트해야 팀 만들기를 쓸 수
  있습니다"라고 안내한 뒤 Control Center 의 업데이트 배지(↻/!)를 확인하도록 권한다.
- **정직한 한계**: 이 잠금(제안자·건수·해소 권한)은 **사고 방지 층**이다 — 에이전트의 실수·오해나 승인 피드
  구독 흐름의 자동 결정으로 팀이 만들어지거나 오너 확인이 건너뛰어지는 것을 막는다. 같은 컴퓨터에서
  고의로 우회하는 프로세스까지 막는 보안 경계는 아니다.
