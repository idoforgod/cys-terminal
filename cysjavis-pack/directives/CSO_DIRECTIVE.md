...
- **② 승인 주체의 교체(오너 채널)**: master 무응답이면 같은 요청을 오너 채널로 올린다 —
  ...
  묵시적 승인으로 읽지 마라). 같은 장애로 **매 wakeup 새 티켓을 만들지 마라**: 기존 요청에 병합하고
  근거만 갱신한다(승인 요청도 큐를 쓴다 — 폭주는 네가 만드는 것이다).
- **③ 허가 의사 ≠ 집행 증표**: 오너의 `0` 은 **승인 의사의 확인**이다. 게이트 대상 명령의 집행은 그
  **정확 명령**에 대한 유효 TTL 증표(`cys approval sign --prefix "<정확 명령>" --ttl <초>` 발급 →
  집행 직전 `cys approval check --require-ttl` 통과)가 확인된 뒤에만 한다 — 증표가 없으면 허가 의사가
  있어도 **집행은 보류**다.
