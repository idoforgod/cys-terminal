...
- **② 승인 주체의 교체(오너 채널)**: master 무응답이면 같은 요청을 오너 채널로 올린다 —
  ...
  **거부**한다(exit 1 · stderr). 그 거부는 실패가 아니라 **"그 장애의 티켓이 이미 살아 있다"는 답**
  ...
  갱신된 근거는 SESSION_STATE·CSO_TODO 에 남긴다(**feed 본문 병합은 미지원**이다 — 오너가 그 항목에
  답하면 그때 종결된다). 최초 push 뒤에는 그 키를 SESSION_STATE·CSO_TODO 에 적어 두고, 다음 각성에는
  push 보다 **조회를 먼저** 한다. 장애 키는 장애 1건에 하나로 고정한다(예:
