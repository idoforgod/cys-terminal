...
- **도구 호출 예산(`tool_calls`)**: 단위는 턴이 아니라 **도구 호출 수**다. 세션당 **1,500회**에서 경고,
  **2,000회**부터 비필수 도구를 deny 한다. 필수 도구는 **예산 deny 에서만 면제**된다(`cys cycle-agent|
  set-status|identify|status|list` · **보고 채널은 `cys send --to master` 와 `cys send --queued --to master`
...
- **② 승인 주체의 교체(오너 채널)**: master 무응답이면 같은 요청을 오너 채널로 올린다 —
  ...
  **해소된 항목도 같은 거부**를 낸다 — 거부는 **티켓 생존의 증거가 아니다**(거부를 '아직 대기 중' 으로
  ...
  (그때만 새 키다). 갱신된 근거는 SESSION_STATE·CSO_TODO 에 남긴다(**feed 본문 병합은 미지원**이다 —
...
- **⑤ 배선 고지(미배선)**: `cys feed push --wait` 는 §1-1 접두 목록에 **아직 없다** — 게이트가 등록된
  설치에서 이 경로가 막히면 ②도 불가다. 그때 네가 할 수 있는 전부는 SESSION_STATE·CSO_TODO 에 교착
  사실·근거·마지막 점검 완료 시각을 남기는 것이며(오너가 읽는 흔적), 그 상태를 '해결됨' 으로 적지
  마라. 이 접두의 등재는 능력 게이트 릴리스의 수용 조건이다.
