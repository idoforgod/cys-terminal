...
describe("main.ts 배선 핀", () => {
  ...
  test("재진입 가드가 첫 await 앞에서 세워지고 finally 에서 풀린다", () => {
    const f = fnBody("runTeamProposalFlow");
    const g = f.indexOf("teamProposalInFlight = true");
    expect(g).toBeGreaterThan(0);
    expect(g).toBeLessThan(f.indexOf("await "));
    expect(f).toContain("finally");
    expect(f.indexOf("teamProposalInFlight = false")).toBeGreaterThan(f.indexOf("finally"));
  });
