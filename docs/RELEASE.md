...
- [ ] **★메인 페이지(`/`) 원격 검증 — 6항목 전부 (S28 + 2026-07-29 오너 지시 ⓐⓑⓒ · 자동화 밖의 수동 게이트)**
      ...
      **이 레인에는 그 두 스크립트가 존재하지 않았다**(실측) — 그래서 메인 페이지 검증은
      ...
      ★**종료코드 계약 (2026-08-25 개정 — 위양성 BLOCK 사고 수리)**: `0` 전건 통과 ·
      `1` **원격이 틀렸다**(발행 미완 = 진짜 적색) · `2` 사용법 · `3` **DOWNLOAD_FAILED —
      게이트 자신의 다운로드가 실패했다**(유계 재시도 3회 소진 · 원격 무결성은 **판정되지
      않았다** → 그대로 재실행하라. 자산 적색이 아니다).
      배경: 구 구현은 자산을 `curl -s`(--fail 없음)로 받아 **종료코드도 수신 바이트수도 보지 않고**
      곧바로 sha256 을 걸어, 끊긴 전송의 부분 본문을 "해시 불일치"(=exit 1)로 찍었다.
      ...
      회귀 자산: `python3 scripts/tests/test_verify_release_remote.py`(19건 · 네트워크 불요). 그래서 메인 밴드 누락은
