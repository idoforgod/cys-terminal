...
- [ ] **★메인 페이지(`/`) 원격 검증 — 6항목 전부 (S28 + 2026-07-29 오너 지시 ⓐⓑⓒ · 자동화 밖의 수동 게이트)**
      ...
      **이 레인에는 그 두 스크립트가 존재하지 않았다**(실측) — 그래서 메인 페이지 검증은
      ...
      그 스크립트가 못 돌 때의 폴백이자 판정 기준의 서술로 남긴다.

      ★**종료코드 계약 (2026-08-27 개정 — 위양성 BLOCK 사고 2라운드 수리)**. 판정은 **세 분류**이고
      **요약 첫 줄이 어느 것인지 이름을 댄다**:
      · `0` 전건 통과
      · `1` **적색 — 원격이 틀렸다.** 두 하위 분류가 첫 줄에 찍힌다:
        **자산 무결성 실패**(완전 수신한 바이트 ≠ SHA256SUMS · 미등재) /
        **원격 확정 오류(4xx)**(404 등 — 원격이 "없다"고 확정 답을 했다) /
        **페이지 내용 불일치**(버전·용량·마커 집계 실패)
      · `2` 사용법
      · `3` **미판정(DOWNLOAD_FAILED)** — 이 도구가 그 항목을 **판정하지 못했다.** 수신이
        완결되지 않았거나(전송 중단·부분 본문) 원격이 일시적으로 대답을 못 했고(**HTTP 429·5xx**),
        유계 재시도 3회를 소진한 상태다. ★**적색도 초록도 아니다** — 오리진에 실제로 박힌 부분
        자산과 경로에서 끊긴 전송은 **수신 측에서 같은 증상**이라 이 도구에는 가릴 근거가 없다.
        그래서 문면은 어느 방향으로도 단언하지 않고, 재시도가 매번 같은 지점에서 끝났으면 그
        **관측 사실만** 덧붙인다(해석·원인 지목 없음 — 판단은 사람 몫). 조치: 그대로 재실행.
      배경 ①(1라운드): 구 구현은 자산을 `curl -s`(--fail 없음)로 받아 **종료코드도 수신 바이트수도
      보지 않고** 곧바로 sha256 을 걸어, 끊긴 전송의 부분 본문을 "해시 불일치"(=exit 1)로 찍었다.
      v0.14.27 검증 2차 실행에서 실제로 위양성 BLOCK 이 났다(같은 자산 7회 재실측 전건 일치 =
      자산 무결 · 1·3차 실행은 PASS). 게이트가 무작위 적색을 내면 사람이 **진짜 적색까지
      "또 그거겠지"로 흘린다** — 그래서 두 사건을 다른 종료코드·다른 문면으로 갈랐다.
      배경 ②(2라운드 · 2026-08-27): 그 1라운드 수리가 `status >= 400` 을 통째로 "원격이 틀렸다"로
      접어 **재시도를 한 번도 하지 않았다**. 그 정당화는 4xx 에만 성립한다 — **503·502·504·429 는
      "릴리스가 틀렸다"가 아니라 "지금 대답을 못 한다"**이고, 호스팅/CDN 블립 1회로 릴리스가
      BLOCK 되는 것은 이 수리가 죽이겠다고 선언한 바로 그 클래스였다. 이제 429·5xx 는 전송 실패와
      같은 유계 재시도를 타고 소진되면 **미판정(exit 3)** 으로 접힌다. 아울러 1라운드 문면이 남긴
      **반대 방향의 단언**("이것은 자산 적색이 아니다")도 제거했다 — 그 역시 근거 없는 주장이다.
      회귀 자산: `python3 scripts/tests/test_verify_release_remote.py`(30건 · 네트워크 불요). 그래서 메인 밴드 누락은
