...
> 이번 빌드 원본에서 컴파일 시 뽑은 상수 — 훅 `!getdllversion` :84-85 · `CYS_ORACLE` :131-149)
> 이며, 배치는 `<bin>.new.exe` 추출→검증→**rename 2단**의 3종 트랜잭션(`CYS_PLACE` :184-321 ·
> 순서 cys→cysd→cys-app :567-571)이다. 기대값의 동결 스키마는
...
> **rename 2회**(정식→prev 슬롯 `cys_pl_vacate` 훅 :238-279 → `.new`→정식 `cys_pl_fill` :280-283)
> 로 교체한다. 두 rename 은 모두 메타데이터 연산이라 정식 이름이 비는 창은 **수 ms** 다
> (훅 L2 :36-38). 종전(0.14.27)의 `Rename → cmd copy` 2단이 만들던 수십~수백 ms 창보다 더
> 좁아졌고, 같은 버전 재설치는 오라클 단락(`CYS_PLACE` ① :187-193)으로 **아예 스왑을 하지
> 않으므로** 창 자체가 없다. 200ms 표집에서 False 0~2줄은 정상 범위다.
>
> 채우기 rename 이 일시 거부되면(AV 가 `.new` 를 쥔 경우) 유한 재시도(5회×1초 —
> `CYS_RENAME_RETRY` :109-122)가 도는 동안 창이 최대 ~5초까지 늘 수 있으나, 그 종단은
> 반드시 **prev 슬롯 복귀**(`CYS_RESTORE_SLOT` :159-177) 후 `placement-refused` 유음 실패다
...
> :474-484)과 최종 바닥 점검(`CYS_LASTDITCH` :370-438), 그리고 다음 cysd 부팅의 회수 가드
> (P11)가 `.new`(신본) → prev 체인(구본) 순으로 자리를 닫는다.
...
Get-ChildItem $INST -Filter '*.new.exe'        # ★비어 있어야 한다(성공 배치는 rename 으로 소멸 + Delete 벨트 · 훅 :313-318)
```
...
> 훅 :84-85 · 런타임 `CYS_ORACLE` :131-149 · 배치 후 재검증 :292-296). 읽기 실패·리소스
> 부재는 **fail-closed**(신본 아님으로 간주 :136-137·:142-143)다. 따라서:
> - 신본이 실제로 안 들어왔는데 성공(exit 0)으로 끝나는 조합은 원리적으로 없다 — 보이면
>   오라클 자체 결함이니 **최우선 보고**.
> - **같은 버전 재설치는 오라클 단락**(:187-193)으로 아무 스왑 없이 exit 0 이다 — prev
>   슬롯이 새로 생기지 않는 것이 정상이다(면제 기계 불요의 근거 · NSIS-CONTRACT §4).
> `cys-installed-version.txt` 는 이제 판정 재료가 아니라 **정보성 마커**다(훅 :649-656) —
...
| 결과 | 판정 |
|---|---|
| 둘 다 True 이고 `cys --version` 이 정상 출력 | **PASS** — 중단이 '빈 자리'가 아니라 '동작본'을 남겼다(콜백 `CYS_ABORT_RESCUE` :445-470 이 `.new` 신본 → prev 구본 순으로 자리를 닫으므로 신·구 어느 쪽이어도 정상) |
...
**기대 동작**
- 설치기가 마지막에 **실패**로 끝난다(빨간 중단 · GUI 면 `cys installation did not complete correctly.` 메시지 상자 · 훅 :635).
- `%LOCALAPPDATA%\cys\cys-install-failure.txt` 가 생긴다(훅 :613-624). 1행은 종전과 같은
  ...
  **토큰 4줄이 항상 존재**한다(해당 없으면 접두어 뒤 빈 값 — 동결 스키마 NSIS-CONTRACT §3):
  ```
  unrecoverable:  ← 이 시험에서는 cys.exe 가 여기에 적힌다(0바이트 자리를 어떤 재료로도 못 메움 · CYS_LASTDITCH :433-435)
  rolled-back-to-previous:
  not-updated:
  placement-refused:  ← cys.exe(vacate-locked) 도 함께 적힌다(읽기전용 ACL 이 rename 대피를 거부 · :273-276)
  ```
- cysd.exe·cys-app.exe 는 토큰 목록에 **없다** — 배치는 cys 부터의 고정 순서 트랜잭션이라
  (:567-571) 첫 바이너리 거부 시 나머지는 시도조차 하지 않는다(시도 안 한 것은 목록에 없음 —
...
> 종료코드 규약(훅 :606-610 · NSIS-CONTRACT §4): **3 = 정식이 없(었)거나 구본으로 비상
> 복구됨**(`unrecoverable:`/`rolled-back-to-previous:` 비어있지 않음) · **4 = 구본 무손상·
> 동작, 신본만 미반영**(`not-updated:`/`placement-refused:` 만 비어있지 않음 — P6/P10 이
> 이 쪽을 시험한다). 둘 다 "당신의 cys 는 여전히 동작하고 세션도 안 죽었다" 가 전제다 —
...
# 다음 완주가 스스로 지우고 새로 추출하므로(훅 ② :195-199) 손대지 않아도 된다.
...
> 읽기 프로브(:135-137)는 통과시키고 rename 만 거부하는, 실세계 AV 잠금의 재현이다.
...
:691-693 · `<bin>.prev*.exe`(고정 슬롯 + tick 슬롯) :694-696 · 마커 2종 :697-698 ·
...
> 프로세스는 제거기의 고정 kill 목록(:670-687) 밖이라, 그 파일 하나가 삭제를 거부하고 남을
...
:653), 판정은 정식 파일의 VERSIONINFO 를 직접 묻는 절대 오라클(:131-149)이다. 그 설계에서
"구본 잔존 + 성공 보고" 조합은 종단이 없다: 오라클이 notfresh 로 판정한 바이너리는
성공(스왑 완료·재검증 통과 :292-296)하거나, 토큰과 함께 exit 3/4 로 거부된다. 이 항목은
그 오라클의 **양방향**을 실기로 잰다.
...
$p = Start-Process $setup -ArgumentList '/S' -PassThru -Wait; $p.ExitCode   # ★0 기대(오라클 단락 :187-193)
Test-Path "$INST\cys-install-failure.txt"                                   # False
...
$p = Start-Process $setup -ArgumentList '/S' -PassThru -Wait; $p.ExitCode   # 0 기대(notfresh → 스왑 :194-321)
...
배치의 정식 부재 창은 "정식→prev rename(:238-279)" 과 "`.new`→정식 rename(:280-283)" 사이다.
정확히 그 순간 설치기가 죽으면(전원 차단·강제 종료) 디스크에는 **정식 없음 + `.new`(신본) +
prev(구본)** 가 남는다 — 훅의 콜백(:474-484)은 프로세스가 죽으면 돌 수 없으므로, 이 상태의
...
| 갭 | 상태 | 근거 |
|---|---|---|
| 정식 이름이 **빈 채로 남는 종단** | **닫힘(코드·3중)** | ①배치 실패 전수 슬롯 복귀(`CYS_RESTORE_SLOT` :159-177·undo :323-354) ②콜백+최종 바닥 점검(:370-438·:474-484) ③cysd 부팅 회수 가드(Wave 1 W1 — 설치기 밖 2차 복구선). P1·P4·P11 이 실기로 잰다 |
| **구본 잔존 미탐**(반쪽 업그레이드를 성공 보고) | **닫힘(코드·설계 교체)** | 지문 대조 폐기 → 절대 오라클(VERSIONINFO == 컴파일 상수 :131-149) + 배치 후 재검증(:292-296). notfresh 종단은 전부 exit 3/4 + 토큰. P10 이 양방향을 잰다 |
| **같은 버전 재설치 → 영원한 exit 4 루프**(2026-08-28 실사고) | **닫힘(코드)** | 면제 기계 제거 + 오라클 단락(:187-193) — "이미 이번 빌드" 는 무행동 성공. P10-a·CI T4-4 |
| **이미지 잠금 하 업그레이드 실패**(잠긴 구 CLI 위로) | **닫힘(코드)** | `.new` 추출→rename 2단은 잠긴 정식을 덮지 않고 대피시킨다(:238-283). P6-b·CI T4-14 |
| **전이 창 자체** | **좁힘(rename 2회 사이 수 ms)** | copy 채움 폐기 — 채움도 rename(:280-283). copy 는 undo 복귀 경로에만 잔존(:162). 잔여 창은 P11 의 부팅 가드가 받친다 |
| 핸들 잠금(delete 비공유) 기계 | **의도된 거부(exit 4)** | rename 조차 불가한 잠금은 우회 불가 — 구본 무손상으로 거부하고 재실행 안내(:273-277·:620-621). P6-a |
| 스테이징 사본이 **추출 후에 손상**되는 경우(AV 절단 등) | **좁힘** | `.new` 검증(존재·≥64KiB·버전 :210-222) + 배치 후 오라클 재검증(:292-296)이 잡는다. VERSIONINFO 리소스가 온전한 채 본문만 손상된 파일은 여전히 통과 가능 — 잔여 갭(정직 고지) |
