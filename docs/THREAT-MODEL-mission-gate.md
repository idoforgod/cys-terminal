...
2-b. **부분 포함 과확장(수용 · R5 · ★R6 에서 좁힘)** — 오너가 기계 배달 문안(24자 이상)을 그대로
   ...
   **오너의 평범한 문장**(마크다운 구분선·같은 글자 반복)이 통째로 차단됐다 — 이 하한의 목적과
   ...
   하한 이상**일 때만 유효하다. 전량 커버(concat)는 길이·동일성과 무관한 규칙이라 이 정정으로
   방어가 약해지지 않는다(같은 코퍼스로 양성/음성 동시 실측).
...
7. **멀티라인 분할의 잔여 (R6-A 봉합 · 아래 셋은 미해소)** — §1 #17 을 닫는 것은 **데몬이 남기는
   ...
   · **ⓑ 조각 상한(`delivery.rs::MAX_PARTS`)을 넘는 초장문 push** — 넘긴 행은 원장에 없다.
     조용히 넘기지 않고 `delivery.parts_incomplete` 이벤트로 남긴다(차단이 아니라 탐지 · §3).
