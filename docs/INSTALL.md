...
1. **권장 — GUI 1클릭(1회 관리자 승인):** Control Center 헤더 → **"셸에 cys 설치"** 클릭 →
   ...
   - **그 자리에 이미 무언가 있을 때 — 무엇이든 백업합니다.** `/usr/local/bin/cys`(또는 `cysd`)
     ...
     - 그런 것이 있으면 버튼을 누르기 전에 Control Center에 미리 고지가 뜨고, 그 고지 문장에도
       "실제 파일이든 링크든 백업한다"는 같은 내용이 적혀 있습니다(화면과 문서가 같은 말을
       하는지 자동 테스트가 지킵니다).
   ...
   - **Control Center 알림의 제목은 "실제로 무엇이 있는가" 만 말합니다.** 위의 안내들
    ...
    그 알림의 제목은 세 갈래이고, 갈래를 정하는 것은 문장이 아니라 **실제로 관측된 상태**입니다:
    - `/usr/local/bin` 에 **이 앱의 것이 아닌 파일이 실제로 있을 때만** — "⚠ /usr/local/bin 에
...
> 않고, 화면에도 되돌리는 명령(`sudo mv …`)을 만들어 주지 않습니다 — 추측한 경로로 파일을
...
어느 쪽 복구 명령을 따라야 하는지 알기 어려웠습니다. 되돌리는 명령(`sudo mv …`) 문장은 앱 화면이
직접 조립합니다 — 같은 사실을 두 곳에서 서로 다르게 쓰지 않기 위해서입니다.
