...
- (mac 소비) alt 진입 시 희망 트래킹 재생 주입(1049h 방출 직후 위치)·alt 중 통과·복귀 시 MOUSE_PARAMS 8종 전 집합 상수 DECRST·스트리핑 재개. **상수 주입은 필터 우회 term.write 직접**(자기 스트리핑 차단). Windows=장부 기록만 공통·소비 전부 비활성(win-parity 바이트 핀 + ~~휠 핸들러 win 미등록 단언~~ → **본문 정정 2026-08-17: win 에도 별도 술어(`shouldSuppressWheelWin`)로 등록한다** — 아래 ★정정 문단이 이유다. 종전 구를 append-only 로만 뒤집어 두면 첫 문장에서 멈춘 독자가 오도되므로 구 자체를 고쳤다). OS는 trackfilter 생성자 opt로 캡처 주입. **★정정(2026-08-17 실측 — 이 절의 win 서술 전체에 우선한다)**: 이 설계가 깔고 있던 "win claude 는 기본 inline 이라 문제2가 미발현" 전제는 **반증됐다**. Claude Code 2.1.233 의 fullscreen 판정 함수 `ra()` 에 **순수 Windows→inline 분기가 없고**, Windows 관련 분기는 `Windows ∧ SSH` 하나뿐이며, settings 의 `tui` 키가 없으면 최종 판정은 **서버측 기능 게이트**가 한다 — 즉 **OS 가 아니라 계정·롤아웃이 결정한다**. 저장소에도 이 전제를 뒷받침하는 버전 핀·감지 코드는 **0건**이었다. ∴ win 소비 비활성의 정당한 근거는 '미발현'이 아니라 **ConPTY 결함 회피**(마우스 보고가 입력창에 리터럴로 타이핑됨)이며, 그래서 남는 win 잔여 위험(alt buffer 휠→방향키 합성)은 **입력을 앱에 열지 않는 별도 억제 술어**로 막는다(출력 소비 재활성화·킬스위치 재사용은 금지 — 그것이 ConPTY 결함 1호를 되살린다).
...
### D5 — env 게이트·관측
- 주입: agent_env_pairs 단일 SOT — 키 부재 시에만 (CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN,"1") 삽입(contains — append+sort로 사용자 "0" 뒤집기 금지). 게이트=~~cfg!(macos) ∧ extract_bin(cmd)=='claude'~~ → **본문 정정(2026-08-17 · 같은 날 2차 개정): `d5_gate_for_os(OS, d5_win_opt_in()) ∧ extract_bin(cmd)=='claude'` 이고 매핑은 `macos=무조건 · windows=옵트인 시에만 · 그 외=거짓` 이다**(구 `cfg!(macos)` 는 코드 어디에도 남아 있지 않다 — 같은 문서 D4 절의 교정 방식을 따라 **구 자체에 취소선을 그었다**: append-only 로만 뒤집어 두면 첫 문장에서 멈춘 독자가 오도된다. 1차 개정은 `macos∨windows` 무조건이었으나, 앵커 ④(전 pane 사망) 위험 + Windows 실기 스모크 B-5 미수행을 이유로 Windows 를 **옵트인으로 강등**했다. 옵트인 채널 = `~/.cys/win-no-alt-screen` 파일 · `CYS_WIN_NO_ALT_SCREEN=1`. 근거·승격 절차·회귀 핀은 `src/lib.rs` 의 `d5_gate_for_os` doc 이 정본). 이 줄을 근거로 mac 단독 게이트를 복원하지도, 실기 검증 없이 Windows 무조건 주입으로 되돌리지도 마라. 회귀 핀은 두 소비처(인라인 문자열·surface.create env 맵) 모두. **핀 활성화 순서**: 주입 로직 lib 헬퍼화→기존 --lib 레인 편승을 1순위, `cargo test --bin cys` 전체 활성화는 ci-branch 선행 1사이클 green 후 release 승격(112개 휴면 테스트 직행 금지·--skip 패턴 예약).
...
- A13(win): mkdir 원자 락 폴백. Windows 검증 절 — T3 확장(D3 거부 4종 — **launch --help 기대값은 'name 위치=exit 2·부작용 0'으로 갱신**·P0 전 PASS·CEO_TEMPLATE 바이트 대조·**승격 스모크(마커 시드→promote→cmp→down 원복)**·alt_screen·win-parity·agents "0" override)를 릴리스 차단 게이트로 하되 **태그 파이프라인과의 기계 결합(또는 런북 수동 순서)을 명시 배선**. win fullscreen=비지원 선언+힌트(**'옵트인' 표현은 D4 절 ★정정에 종속 — 사용자가 옵트인하지 않아도 계정·롤아웃 판정으로 발현할 수 있다**).
