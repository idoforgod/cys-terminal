...
# 0.14.2 수리 세대 마커 — cysd 소스의 FIX_GENERATION const 바이트열(live RPC 참조+main() 부팅로그 임베드 · v4).
...
# 전 플랫폼 컴파일이라 맥 로컬 릴리스 빌드에서도 사전 검증된다. 0.14.1 이하엔 존재하지
# 않는 신조어 바이트열(0.14.2 세대는 b1b2) — 판별력은 CI 음성 대조(0.14.0 실물 0/1 단언)가 계속 증명한다.
FIX_MARKERS = [
    b"cys-fix-b1b2-gen-0.14.2",
]
