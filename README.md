...
## 외부 정면 비교 — Coral AgentRadio (2026-08 전수조사)

같은 개념 — "일하면서 동시에 듣는" 수동적 인지(passive awareness) 방송 계층 — 을 학계는 실험으로, cys는 제품으로 만들었습니다.
[Coral-Protocol/AgentRadio](https://github.com/Coral-Protocol/AgentRadio)([arXiv:2607.28430](https://arxiv.org/abs/2607.28430)의
재현 레포)를 **파일 전량 정독·실측**하고 독립 적대 리뷰 2회를 거친 비교 요약입니다.
기준일 2026-08-14 (cys v0.14.15 ↔ AgentRadio HEAD 5e4e137).

> **전제(정직 고지)** — 한쪽은 논문 실험용 코드(실행 코드 3,318줄), 한쪽은 2개월 가동 제품(실행 코드 151,307줄)입니다.
> 체급이 다른 비교이므로, 아래는 승패 선언이 아니라 **축별로 무엇이 얼마나 다른지**를 실측으로 적습니다.

### 자비스가 앞서는 것 — 판정 8축 중 6축 (전 항목 실측)

| 축 | cys/자비스 | AgentRadio |
|---|---|---|
| 아키텍처(영속·복구) | radio 상태 = **파일 정본**(세션 clear·컨텍스트 압축 생존) · ack 커서 복구 · 멱등키 중복 방지 | 서버 **JVM 메모리**(죽으면 전 대화 소실) · 중복 방지 없음 |
| 기능 폭 | CLI 66종 · RPC 68종 · 결정론 도구 86종 · 스킬 114종 | 통신 프리미티브 3종 + 상태읽기 |
| 신뢰성·검증 | 자동 테스트 **1,710케이스** · CI 5본 · radio 어서션 297 전건 PASS | 자체 테스트 **0** · CI **0** · 채점기 3중 사본 불일치 |
| 보안 | fail-closed 이중 서명 · 커널 유래 발신자 신원 · ACL · 발행 전 비밀 스캔 | 고정 크리덴셜 `test` · 무체크섬 106MB JAR · 전 에이전트 권한 우회 모드 |
| 운영·관측 | 관제 9탭 · 프로세스 원장 · 워치독 · 사전 자원 게이트 | 계측 함수 no-op(토큰 사용량 미회수) |
| 성숙·배포 | 2개월 149릴리스 · macOS 공증 DMG · 무중단 팩 업데이트 | 릴리스 1건(v1.0.0) |

방송 계층 정면 대조(17항목)에서도 **내구성·보안·검증 계열 11항목은 radio(자비스) 우세** — 거짓 정보의 기계 검증·자동 강등,
2단 비밀 마스킹, 4등급 긴급도+쿨다운 같은 장치는 AgentRadio에 대응물이 없습니다.

### AgentRadio가 앞서는 것 — 2축 + 3항목 (그대로 인정)

| 그들의 강점 | 내용 |
|---|---|
| 공개 벤치마크 점수 | SWE-Atlas QnA 124문제: 단독 32.3% → 4에이전트 62.1% 발표(arXiv+언론). **cys는 공개 벤치 점수가 아직 없음** |
| 외부 노출·생태 | arXiv 논문 계보 3편 · VentureBeat 보도 · 6개 국어 README · MCP 생태(coral-server) |
| 무개조 이식성 | 셸 스크립트 복사만으로 임의 하네스에 적용 — cys의 radio는 cys 스택 필요 |
| 멘션 즉시 깨움 | 서버 push 즉시 반환 — radio 일반 경로는 5초 폴(최긴급 BLOCKER만 stdin 직배달) |
| 이종 모델 실증 | Opus·DeepSeek 두 계열에서 같은 방향의 효과 재현을 시도·보고 |

단 62.1%는 그대로 인용할 수 없는 숫자입니다 — 전수조사로 확정한 할인 사유 5건: 집계 코드 부재(재현 불가),
결과표 산술 오류 1건의 6개 국어 전파, 채점기 관대 편향 2건, 리더보드 미등재(자체 보고), 인접 점수들과 통계적 비구별.

### 우리 부족분 — 같은 잣대로 공개

Windows Authenticode 미서명(인증서 미보유) · 공개 벤치마크 점수 부재 · 외부 채택 지표 미측정 · 아키텍처 문서 1건 수치 드리프트.
벤치 점수는 동일 공개 하네스(Harbor)에서 producer≠evaluator 원칙으로 확보하는 것이 다음 과제입니다.
