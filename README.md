...
| # | 비교 축 | 판정 | 한 줄 근거 |
|---|---|:---:|---|
| 1 | 통신 (수동적 인지) | ⚠️ 조건부 우세 | 3 프리미티브 이식 + 방어 14종·정확히-1회 표면화·철회 시 오염 연쇄 폐쇄·멱등 큐 — 단, **개념의 원조와 실전 데이터는 AgentRadio** |
...
> 서사 쪽(현 리더보드 단일 1위 63.17%가 이미 62.1%를 상회, AgentRadio는 미등재
> 자기보고). 비용도 저자 공개 수치로 태스크당 $2.96 → $19.45(6.6배)입니다.
...
| 통신 표면적 | 프리미티브 3종 | CLI 서브커맨드 67종 (radio 하위명령 17·exit 계약 10종 포함) |
...
| 중복·유실 | 멱등키·ack·순번 없음(메시지 감지는 grep 문자열 세기) — 중복 제거를 LLM 인지에 위탁 | 단조 seq + 표면화/수용 커서 분리 — "0회 금지 > 1회 초과 금지" 불변식 위계 |
