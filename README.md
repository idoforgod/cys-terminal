...
| 4 | 기타 정직 고백 | 비용 공표치 없음(상대는 6.6배까지 공개) · Windows Authenticode 미서명 · radio 자체 문서도 완전한 exactly-once·무청각 0은 보장 못 함을 자인(격상은 부분적) |
...
NousResearch **Hermes Agent**(자가개선 개인 에이전트 플랫폼 · 스타 230,268 · 실행 코드 1,377,316줄 —
2026-08-14 실측/GitHub API)를 저장소 전수조사 + 독립 적대 리뷰 2기로 비교했습니다. AgentRadio 비교와
반대로 **체급은 상대가 큽니다**(코드 약 9.1배 · 테스트 케이스 약 20배) — 그 사실을 먼저 쓰고, 자비스의
우위는 반증 시도에서 살아남은 것만 싣습니다. 9축 판정 **자비스 1승 · Hermes 4승 · 분할 3**, 동층위
(멀티에이전트 오케스트레이션·신뢰 계층) 정면 대조 14항목 **자비스 6 · Hermes 5 · 동급 3**. 자비스의
6승은 전부 **신뢰·통제** 계열, 상대의 5승은 전부 **경제·편의·개방** 계열입니다 — 한 줄로,
**"그들은 능력을 넓혔고, 우리는 절차를 잠갔다."**

### ✅ 자비스가 이기는 것 — 신뢰·통제 6

| # | 능력 | 사실 (실측) |
|---|---|---|
| 1 | 에이전트 간 방송 (수동적 인지) | JavisRadio의 대응물이 Hermes **코어에 부재**(전수 grep — 모든 broadcast는 사람 채널·UI 대상). 최근접물은 태스크 스코프 블랙보드(CLI 전용·폴링) |
| 2 | 자식의 능동 상향 발신 | Hermes 자식은 `send_message` 박탈 — 부모가 로그를 당겨볼(tail) 수만 있음. 자비스 워커는 master stdin에 직접 push(새 턴) |
| 3 | 검증 게이트의 상시·차단성 | 자비스: 증거 없는 완료=exit 5 거부 · 4자 수렴 게이트 · FACT 방송 4단 기계검증. Hermes: 검증 스킬은 실재하나 **선택형**, 완료 시점 상시 장치는 "never blocks completion"을 자기 선언한 비차단 넛지 |
| 4 | 판정단의 이종성·상주성 | 이종 3사(Claude·Gemini·Codex) 리뷰어가 **상주 좌석**+게이트 배선 vs MoA 다사 자문(판정 게이트 아님)·타사 CLI 하청 스킬 — 단 우리도 실가동 통계 미제시(정직 고지) |
| 5 | 에이전트의 자율 조직 | 자비스 master는 티켓 생성→워커 기동→수렴 조회 전 과정을 도구로 자율 수행. Hermes의 Swarm 위상(검증자·합성자)은 CLI 전용 — 모델 미노출 |
| 6 | 사전 자원 통제 + 서명 사슬 | 착수 전 자원 게이트(exit 2=금지)·프로세스 원장 / macOS 공증+minisign **fail-closed 자동 CI** vs 서명·공증 CI 배선 0건(공증 스크립트는 자격증명 없으면 **fail-open** 스킵) |

### ❌ Hermes가 이기는 것 — 숨기지 않고 전부

| # | 능력 | 사실 |
|---|---|---|
| 1 | 규모·기능 폭 | 실행 코드 9.1배 · 테스트 약 20배 · 도구 87(+MCP 무제한) · 스킬 197 · 메신저 22플랫폼 · 실행 백엔드 7종 · 인터페이스 8종 |
| 2 | 실행 보안·샌드박스 | 승인 엔진 4,919줄(하드라인 12+위험 99 패턴·역난독화·LLM 심판) · 백엔드 격리 · 시크릿 소스 통합 — 코드 서명만 우리 우세 |
| 3 | 위임 경제·정책 | 위임 브랜치별 비용·토큰 롤업 실구현 / 서브에이전트 위험명령 **기본 거부**(우리는 자동 승인이 산문 규약) |
| 4 | 연속성·상호운용 | CLI↔TUI↔데스크톱↔메신저 동일 세션 / A2A v1.0 · MCP 서버 · ACP 개방 표준 3종(+피어 인증·감사로그 7층) |
| 5 | 성숙도·생태계 | 스타 230,268 vs 27 · 계정 기여자 398 vs 사실상 1인 · 388일 vs 62일 · 커밋 22,240 vs 528(동일 기저) |

### 정직 고지

벤치마크 점수는 **양측 모두 없습니다**(상대는 README 264줄에 성능 수치 주장 0건 — 과장 없는 정직 ·
우리는 JAVIS-BENCH 착수). 상대에게도 검증 문화가 실재합니다(인용 원장 스킬 grounded-citations ·
독립 리뷰어 스킬 · 이례적으로 정직한 위협 모델) — 초안의 "검증 0건" 표현은 적대 감사에서 반증되어
철회했고, 그 기록까지 보고서에 남겼습니다. 우리 약점도 같은 잣대로: 버스팩터 ≈1 · 개방 표준
(MCP 서버·A2A·ACP) 미노출 · radio 노드 인증 없음 · 공개 벤치 부재. 두 시스템은 대체재가 아니라
보완재입니다 — **결과를 믿어야 하는 일**은 자비스의 6개 능력이, **어디서나 싸게 돌리는 일**은 상대의
5개 능력이 맡습니다. 전 수치·근거(파일:라인)·리뷰 기록: 레인 보고서
