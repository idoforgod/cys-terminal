...
| 3 | 검증 게이트의 상시·차단성 | 자비스: 증거 없는 완료=exit 5 거부 · 4자 수렴 게이트 · FACT 방송 4단 기계검증. Hermes: 검증 스킬은 실재하나 **선택형**, 완료 시점 상시 장치는 "deliberately passive… never blocks completion"(의도적 비차단)을 자기 선언한 넛지 |
...
| # | 능력 | 사실 | 근원(실측 — 보고서 §7-1) |
|---|---|---|---|
| 1 | 규모·기능 폭 | 실행 코드 9.1배 · 테스트 약 20배 · 도구 87(+MCP 무제한) · 스킬 197 · 메신저 22플랫폼 · 실행 백엔드 7종 · 인터페이스 8종 | ⚖️ 혼합 — 메신저 접점은 안 만든 게 아니라 승인·보고 전용 브리지 2종으로 **좁게** 만듦(아래 '채널 브리지' 절). 방향 몫=데이터 주권 아래 폭을 2로 잡은 것, 부채 몫=폭·음성·모바일 열세. 코드·테스트 배수는 시간·인원의 함수 |
| 2 | 실행 보안·샌드박스 | 승인 엔진 4,919줄(하드라인 12+위험 99 패턴·역난독화·LLM 심판) · 백엔드 격리 · 시크릿 소스 통합 — 코드 서명만 우리 우세 | ⚖️ 혼합 — 네트워크 표면은 우리가 작으나(리스너 0), 승인 엔진의 대상은 침입자가 아니라 **에이전트 자신**이라 권한 표면의 부채는 우리 몫이 큼 — 역난독화·인젝션 스캐너 이식이 로드맵 |
| 3 | 위임 경제·정책 | 위임 브랜치별 비용·토큰 롤업 실구현 / 서브에이전트 위험명령 **기본 거부**(우리는 자동 승인이 산문 규약) | 정책=⚖️ 혼합(방어를 착수 전·산출물 후에 배치한 것은 선택이나, 제품의 선언 철학은 HITL+서명 프리픽스라 그 격상 미완은 부채) / 비용 롤업=🔧 부채 |
| 4 | 연속성·상호운용 | CLI↔TUI↔데스크톱↔메신저 동일 세션 / A2A v1.0 · MCP 서버 · ACP 개방 표준 3종(+피어 인증·감사로그 7층) | 연속성=🧭 방향(기기 간 세션 동기화=데이터가 머신을 떠남 — 원칙과 직접 충돌) / 표준=⚖️(A2A만 방향 몫, stdio류 MCP·ACP는 부채 — 로드맵) |
| 5 | 성숙도·생태계 | 스타 230,268 vs 27 · 계정 기여자 398 vs 사실상 1인 · 388일 vs 62일 · 커밋 22,240 vs 528(동일 기저) | ⏳ 시간·인원의 함수 — 개발 모델은 '오너 1인+AI 함대'(달력은 갚아지지 않는 격차임을 그대로 인정) |

### 왜 지는가 — 근원의 실측 분류

지는 축·항목 20개를 양측 설계 문서의 원문 좌표로 분류하면(보고서 §7-1 · 델타 적대 감사 2기 통과):
**🔧 순수 부채 8 · ⚖️ 선택과 부채의 혼합 9 · 🧭 방향(원칙과의 직접 충돌) 2 · ⏳ 시간의 함수 1**.
부채 8건(공개 벤치·Windows 서명·공급망 CI·비용 롤업 등)은 변명 없이 갚을 목록입니다. 나머지의 공통
배경은 설계 중심의 차이입니다 — 우리는 **"로컬 우선 — 데이터는 머신을 떠나지 않는다"**(네트워크
리스너 0 · 커널 유래 신원 · 단일 머신 신뢰 경계)를 중심에 놓았고, 상대는 반대 철학("capability lives
at the edges" · "$5 VPS… not tied to your laptop")으로 접점을 넓혔습니다. 그래서 메신저는 안 만든
것이 아니라 승인·보고 전용 브리지로 **좁게** 만들었고(아래 '채널 브리지' 절), 외부 텔레메트리는 관제
데이터가 머신을 떠나지 않도록 짓지 않았습니다. 같은 잣대를 상대에게도 적용했습니다 — 상대의 '선택형
검증'은 그들의 철학("deliberately passive"·narrow waist)의 결과로 인정합니다(그래도 상시·차단형
게이트 부재라는 판정은 불변). 전 20항목 분류표: 보고서 §7-1.
...
(MCP 서버·A2A·ACP) 미노출 · radio 노드 인증 없음 · 공개 벤치 부재 — 이 중 무엇이 선택이고 무엇이
부채인지는 위 분류와 보고서 §7-1에 실측으로 적어 두었습니다. 두 시스템은 대체재가 아니라
