...
## JavisRadio vs AgentRadio — radio 계층의 출처와 격상점

cys의 수동적 인지 계층(설계 원칙 6 · T5-20)은 독자 발명이 아니라 출처가 있습니다 —
Coral Protocol의 연구 **AgentRadio**(arXiv:2607.28430)는 "4개의 코딩 에이전트가 일을
멈추지 않고 서로의 방송을 듣게 하면"(passive awareness) SWE-Atlas QnA 124태스크에서
단일 에이전트 32.3% → 4에이전트 62.1%가 됨을 통계 검정(McNemar p=0.0023)으로 보였고,
JavisRadio는 그 3 프리미티브를 이식한 뒤 **방송의 신뢰를 프롬프트가 아니라 기계
게이트로** 다시 잠근 재구현입니다. 원 논문·원 소스 전수조사 비교(2026-08-14) 요약:
...
| 검증단 | 동종 모델 4기의 상호 합의 (넷이 같은 착각을 하면 못 잡음) | 이종 3사 리뷰어(claude·agy·codex) · producer≠evaluator 게이트와 결합 |

**원 연구가 앞서는 것도 그대로 적습니다** (이 저장소의 규약): AgentRadio는 공개 벤치마크
124태스크 × 4구성 × 2모델의 ablation 사다리로 자기 방식을 실측 증명했고, 그 측정
엄밀성은 모범입니다. JavisRadio의 현재 증거는 테스트 단언 297("위반이 정확한 exit로
막힌다"를 잠그는 적대 테스트)까지이며 **실전 파일럿은 미가동**입니다(알려진 한계 참조).
참고 맥락: 원 연구의 4에이전트 구성 비용은 태스크당 $2.96 → $19.45(저자 공개 수치)이고,
해당 리더보드의 현재 단일 에이전트 1위(63.17%)는 이미 62.1%를 넘어섰습니다 — "통신
구조가 성능을 올린다"는 원리는 유효하되, 특정 수치 우위의 서사에는 유통기한이 있다는
것까지가 전수조사의 결론입니다.
